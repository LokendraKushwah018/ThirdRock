...
import { GetSettlement,addSettlement } from '../service/adminService';
...
const Settlement = () => {
    ...
    const transction = async (token) => {
        const response = await GetSettlement(token,'1',search);
        if (response.status) {
                                        ...
                                        <tbody>
                                            {data && data.map((option, index) => (
                                                <tr key={index} >
                                                    ...
                                                    <td><span className="font-weight-normal1">{option.marchant_name}</span></td>
                                                    <td><span className="font-weight-normal1">{option.amount}</span>
