...
import { EmployerLoign } from '../service/employService';
...
const Login = () => {
  ...
  const form = useFormik({
    ...
    onSubmit: async values => {
      ...
      const response = await EmployerLoign(data);
      if (response.status == true) {
        if (response.status) {
          toast.success(response.message);
          let employer = {
            user_type: response.user_type == 'EMPLOYER' ? response.user_type.toLowerCase() : 'employer',
  ...
  const forgotPasswordForm = useFormik({
    ...
    onSubmit: values => {
      api.postApi('employer/send-otp-forgot-pass', values).then(response => {
        if (response.status === true) {
          toast.success(response.message);
  ...
  const otpForm = useFormik({
    ...
    onSubmit: values => {
      ...
      api.postApi('employer/verify-otp', values).then(response => {
        if (response.status === true) {
          toast.success(response.message);
  ...
  const resetPasswordForm = useFormik({
    ...
    onSubmit: values => {
      ...
      api.postApi('employer/reset-password', values).then(response => {
        if (response.status === true) {
          toast.success(response.message);
                            ...
                            <div className="card-body">
                              ...
                              <form className="mt-5" onSubmit={form.handleSubmit}>
                                ...
                                <div className="input-group mb-4">
                                  ...
                                  <input type="text" className="form-control" name="mobile_number" {...form.getFieldProps("mobile_number")} placeholder="Please enter mobile number " />
                                ...
                                <div className="input-group">
                                  ...
                                  <input className="form-control" name="password" {...form.getFieldProps("password")} type={values.showPassword ? "text" : "password"} placeholder="Please enter password" id="password-field" />
                                ...
                                <div className="form-group mt-3">Don't have account? <Link to={"/employer/registration"}
