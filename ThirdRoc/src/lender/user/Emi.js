import { Link, useLocation, useParams, useNavigate, Navigate, json } from 'react-router-dom';
...
import { DeleteEmployees, GetEmployer, GetEmployees, GetEmi, payEmis } from '../service/lenderService';
import Index from '..';
...
const Emi = (props) => {
    ...
    const getEmisData = async (token, pageNumber, pageSize, filterVlu) => {
        const response = await GetEmi(token, pageNumber, pageSize, filterVlu, userId);
    ...
    useEffect(() => {
        setpagesCount(1);
        getEmisData(token, pagesCount, 5, search);
    ...
    const payEmisForm = useFormik({
        ...
        onSubmit: values => {
            ...
            console.log('values payEmisForm', values);
            let data = JSON.stringify(values)
            payEmis(token, data).then((res) => {
                console.log('res', res);
                // if (response.status) {
    ...
    return (
        <>
                    ...
                    <div className="main-container container-fluid px-0">
                        <div className="page-header mb-3">
                            <div className="page-leftheader">
                                <h4 className="page-title mb-0 text-primary">Emi Details</h4>
                            </div>
                                            ...
                                            <table className="table table-hover card-table table-vcenter text-nowrap" ref={tableRef}>
                                                ...
                                                <tbody>
                                                    {dataArray && dataArray.length > 0 && dataArray.map((option, index) => {
                                                        ...
                                                        const { customer_id, id, status, amount, first_name, created_at } = option;
                                                        return (
                                                            <tr key={customer_id} >
                                                                ...
                                                                <td>
                                                                    <button className='btn btn-primary' data-bs-target="#pay_emi" data-bs-toggle="modal" onClick={() => setEmiID(id)}>Pay</button>
                                                                </td>
            ...
            <div class="modal fade effect-scale show" id="pay_emi" aria-modal="true" role="dialog">
                <div class="modal-dialog" role="document">
                    <div class="modal-content modal-content-demo">
                        <div class="modal-header">
                            <h6 class="modal-title">Pay</h6>
                            <button aria-label="Close" class="btn-close" data-bs-dismiss="modal" type="button"><span
                                aria-hidden="true">×</span></button>
                        </div>

                        <div class="modal-body application-modal">
                            <form className="mt-5 row" onSubmit={payEmisForm.handleSubmit} >
                                ...
                                <div class="row">
                                    <div class="row g-xs">
                                        <div class="col-12">
                                            <input type="text" placeholder='comment' {...payEmisForm.getFieldProps("notes")} class="form-control" />
                                ...
                                <div class="form-footer mt-2"><button type="submit" class="btn btn-primary" >Submit</button>
                                </div>
                            </form>
                        </div>
                    </div>
