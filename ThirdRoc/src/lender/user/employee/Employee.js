...
import { DeleteEmployees, GetEmployer, GetEmployees } from '../../service/lenderService';
// import Index from '..';
...
const Employee = (props) => {
    ...
    const [amountCreditStatus, setAmountCreditStatus] = useState('');
    ...
    const getEmployeesData = async (token, pageNumber, pageSize, filterVlu) => {
        ...
        const response = await GetEmployees(token, pageNumber, pageSize, queryFilter, filterVlu, userId);
    ...
    useEffect(() => {
        setpagesCount(1);
        getEmployeesData(token, pagesCount, 5, search);
                    ...
                    <div className="main-container container-fluid px-0">
                        <div className="page-header mb-3">
                            <div className="page-leftheader">
                                <h4 className="page-title mb-0 text-primary">Borrower Details</h4>
                            </div>
                                    ...
                                    <div className="row g-xs">
                                        <div className="wd-200 mg-b-30">
                                            <div className="input-group">
                                                <select className="form-control mb-0" name="status" value={queryFilter.filter} onChange={(e) => handleChangeQueryBuilder(e)}>
                                            ...
                                            <table className="table table-hover card-table table-vcenter text-nowrap" ref={tableRef}>
                                                <thead className="border-bottom-0 pt-3 pb-3">
                                                    <tr>
                                                        ...
                                                        <th className="font-weight-bold">TOTAL LIMIT</th>
                                                        <th className="font-weight-bold">AVAILABLE LIMIT</th>
                                                        <th className="font-weight-bold">USED LIMIT</th>
                                                ...
                                                <tbody>
                                                    {dataArray && dataArray.length > 0 && dataArray.map((option, index) => {
                                                        ...
                                                        const { customer_id, mobile_number,first_name, status, available_limit, total_limit, used_limit, company_name, name } = option;
                                                        return (
                                                            <tr key={customer_id} >
                                                                <td>{serial_num}</td>

                                                                {/* <td><span className="font-weight-normal1">{serial_num}</span></td> */}
                                                                <td><span className="font-weight-normal1">{name}</span></td>
                                                                <td><Link to={prefix + '/employee_detail/' + customer_id}><span className="font-weight-normal1">{mobile_number}<br/>{first_name}</span></Link></td>
                                                                <td><span className="font-weight-normal1">{company_name}</span></td>
                                                                <td><span className="font-weight-normal1">{total_limit ? total_limit : 0.00}</span></td>
                                                                <td><span className="font-weight-normal1">{available_limit ? available_limit : 0.00}</span></td>
                                                                <td><span className="font-weight-normal1">{used_limit ? used_limit : 0.00}</span></td>
                                                                ...
                                                                <td>
                                                                    <Dropdown>
                                                                        ...
                                                                        <Dropdown.Menu>
                                                                            ...
                                                                            <Dropdown.Item>
                                                                                {status == 'Unassigned' ?
                                                                                    <Link to={prefix + `/employee_setlimit/${customer_id}/${status}`}>Set Limit</Link>
                                                                                    :
                                                                                    <Link to={prefix + `/employee_setlimit/${customer_id}/${status}`}>Manage Limit</Link>
                                                                                }
                                                                            </Dropdown.Item>
                                                                            {/* <Dropdown.Item>
