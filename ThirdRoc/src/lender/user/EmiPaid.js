...
import { DeleteEmployees, GetEmployer, GetEmployees,GetEmiPaidList } from '../service/lenderService';
import Index from '..';
...
const EmailPaid = (props) => {
    ...
    const getEmiPaid = async (token, pageNumber, pageSize, filterVlu) => {
        const response = await GetEmiPaidList(token, pageNumber, pageSize, filterVlu, userId);
    ...
    useEffect(() => {
        setpagesCount(1);
        getEmiPaid(token, pagesCount, 5, search);
                    ...
                    <div className="main-container container-fluid px-0">
                        <div className="page-header mb-3">
                            <div className="page-leftheader">
                                <h4 className="page-title mb-0 text-primary">EMI Paid Details</h4>
                            </div>
                                            ...
                                            <table className="table table-hover card-table table-vcenter text-nowrap" ref={tableRef}>
                                                ...
                                                <tbody>
                                                    {dataArray && dataArray.length > 0 && dataArray.map((option, index) => {
                                                        ...
                                                        const { customer_id,paid_date,first_name,amount,status,late_charges,total_paid_amount,created_at } = option;
                                                        return (
                                                            <tr key={customer_id} >
                                                                ...
                                                                <td><span className="font-weight-normal1">{late_charges}</span></td>
                                                                <td><span className="font-weight-normal1">{total_paid_amount}</span></td>
                                                                <td><span className="font-weight-normal1">{paid_date}</span></td>
