...
import { LenderLoign } from '../service/lenderService';
...
const Login = () => {
  ...
  const form = useFormik({
    ...
    onSubmit: async values => {
      ...
      const response = await LenderLoign(JSON.stringify(values));
      ...
      // api.postApi('lender/login', values).then(response => {
      //   if (response.status === true) {
      //     toast.success(response.message);
      //     let lender = {
      //       user_type : response.user_type == 'USERS' ? response.user_type.toLowerCase() : 'lender',
  ...
  const forgotPasswordForm = useFormik({
    ...
    onSubmit: values => {
      api.postApi('lender/send-otp-forgot-pass', values).then(response => {
        if (response.status === true) {
          toast.success(response.message);
  ...
  const otpForm = useFormik({
    ...
    onSubmit: values => {
      api.postApi('lender/verify-otp', values).then(response => {
        if (response.status === true) {
          toast.success(response.message);
  ...
  const resetPasswordForm = useFormik({
    ...
    onSubmit: values => {
      api.postApi('lender/reset-password', values).then(response => {
        if (response.status === true) {
          toast.success(response.message);
                          ...
                          <div className="bg-white text-dark br-7 br-tl-0 br-bl-0">
                            <div className="card-body">
                              <div className="text-center mb-3">
                                <h3 className="mb-2">Login with Lender</h3>
                              </div>
                              <form className="mt-5" onSubmit={form.handleSubmit}>
                                ...
                                <div className="form-group mt-3">Don't have account? <Link to={"/lender/registration"}
